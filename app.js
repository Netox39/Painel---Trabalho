...
(function injectUiTweaks(){
  const css = `
    ...
    .drawer .drawerBody{ margin-top:10px; display:flex; flex-direction:column; gap:10px; }
  `;
...
(async function init(){
  ...
  // NAV: deixa o Inventário visível e comprime o restante em uma gaveta "T.I"
  const invIndex = TABS.findIndex(t=>t.key==="inventario");
  const mkNavBtn = (t,i)=>{
  ...
  if(invIndex >= 0){
    nav.appendChild(mkNavBtn(TABS[invIndex], invIndex));
  }

  const drawer = document.createElement("details");
  drawer.className = "drawer";
  drawer.open = true;
  drawer.innerHTML = `<summary><span>T.I</span><span class="badge">Menu</span></summary><div class="drawerBody"></div>`;
  const drawerBody = drawer.querySelector(".drawerBody");

  TABS.forEach((t,i)=>{
    if(i === invIndex) return;
    drawerBody.appendChild(mkNavBtn(t,i));
  });
